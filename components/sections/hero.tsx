...
export function HeroSection({ mouseX = 0.5, mouseY = 0.5 }: { mouseX?: number; mouseY?: number }) {
  ...
  return (
    <section className="relative flex min-h-screen items-center overflow-hidden" style={{ background: "radial-gradient(ellipse at 70% 30%, #5C1535 0%, #3D0D26 50%, #250818 100%)" }}>
      ...
      <div className="absolute inset-y-0 right-0 z-0 w-full [background-position:center_8%] sm:[background-position:center_15%] lg:w-[62%]" style={{ backgroundImage: "url('/imperial ballet/hero.jpg')", backgroundPosition: "center 15%", backgroundSize: "cover" }}>
      ...
      <div className="relative z-10 ml-0 max-w-7xl px-4 pt-40 pb-24 text-left sm:px-6 sm:pt-44 sm:pb-28 lg:mx-auto lg:w-1/2 lg:px-8 lg:pt-48 lg:pb-16">
        ...
        <h1 className="mb-6 text-left font-serif text-4xl leading-[1.02] font-bold text-balance text-[#F5F5F5] italic sm:text-5xl md:text-6xl lg:text-[4.25rem] xl:text-[4.75rem]">
          <span className="animate-clip-reveal inline-block">Coltiva il talento.</span>{" "}
          <span className="animate-clip-reveal inline-block" style={{ animationDelay: "0.15s" }}>Esprimi la tua passione.</span>{" "}
          <span className="animate-clip-reveal inline-block" style={{ animationDelay: "0.35s" }}>Vivi la danza.</span>
        </h1>
        ...
        <p className="animate-clip-reveal mb-10 max-w-none text-lg leading-relaxed font-light text-pretty text-[#B8B8B8] sm:text-xl lg:max-w-xl lg:text-2xl" style={{ animationDelay: "0.65s" }}>
          Diretta da Melania e Rossella Mellino, la nostra scuola offre un ambiente professionale, accogliente e stimolante dove ogni allievo può sviluppare tecnica, disciplina, sicurezza e amore per la danza.
        </p>
